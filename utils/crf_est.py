...
class crf_est:
    """
    Class for estimating the camera response function (CRF) from a stack of images.
    """
    def __init__(self, times=None, n_samples=0, sampling_strategy=None, smoothing_term=128, bNormalization=1):
        """
        Initialize the CRF estimator.
        Parameters:
            times : list of exposure times for each image in the stack.
            n_samples : number of samples for the CRF estimation.
            sampling_strategy : strategy for sampling the images.
            smoothing_term : smoothing term for the CRF estimation.
            bNormalization : flag to normalize the CRF.
        """
        if n_samples < 1:
            self.n_samples = 256
        if times is None:
...
def process_case(args):
    ...
    stack_path = []
    for i in range(0, 4):
        stack_path.append(os.path.join(case_path, f"inpaint/-{i}.png"))
    ...
    t = np.array(times)
    if MERGE:
        test_case = os.path.basename(case_path)
        # inverse_crf = scipy.io.loadmat(f'/ssddisk/ytlin/data/HDR-Real/CEVR/{test_case}/response.mat')['lin_fun'].reshape((256, 3)).astype(np.float32)
        ...
        imgOut = np.zeros_like(stack[0].astype(np.float32))
        total_weight = np.zeros_like(stack[0].astype(np.float32))

        for i in range(len(stack)):
            weight_type = 'DEB97'
            tempstack = np.clip(stack[i].astype(np.float32) / scale, 0, 1)
            if(i == i_sat):
            ...
            # linearize the image
            tempstack = removeCRF(tempstack, output)
        ...
        imgOut = np.divide(imgOut, total_weight, where=total_weight>0) 
        imgOut = np.exp(imgOut)
        saturation = 1e-4

        # saturation check
        if np.sum(total_weight <= saturation) > 0:
            i_med = np.round(t.shape[0] / 2).astype(np.int8)
            med = np.clip(stack[i_med] / scale, 0, 1)
            tempstack = np.clip(stack[i_sat].astype(np.float32) / scale, 0, 1)

            img_sat = removeCRF(tempstack, output)
            img_sat = img_sat / t[i_sat]
        ...
        cv2.imwrite(f'/home/ytlin/boosting_HDR/results_clip/VDS/glowgan_boost/{test_case}/inpaint/hdr.hdr', hdr)
...
if __name__ == "__main__":
    # Example usage
    times = [8, 4, 2, 1]
    path = '/home/ytlin/boosting_HDR/results_clip/VDS/glowgan_boost'
